...
var COL_MIN_WIDTH, Calendar, CalendarExtension, Class, ClippedScroller, ComponentFootprint, CoordCache, DEFAULT_GRID_DURATION, DragListener, EmitterMixin, EnhancedScroller, EventDef, EventDefMutation, EventFootprint, EventRow, EventSource, FC, Grid, HRowGroup, LICENSE_INFO_URL, ListenerMixin, MAX_AUTO_CELLS, MAX_AUTO_SLOTS_PER_LABEL, MAX_CELLS, MIN_AUTO_LABELS, PRESET_LICENSE_KEYS, Promise, RELEASE_DATE, Resource, ResourceAgendaView, ResourceBasicView, ResourceComponentFootprint, ResourceDayGrid, ResourceDayTableMixin, ResourceGridMixin, ResourceManager, ResourceMonthView, ResourceRow, ResourceTimeGrid, ResourceTimelineGrid, ResourceTimelineView, ResourceViewMixin, RowGroup, RowParent, STOCK_SUB_DURATIONS, ScrollFollower, ScrollFollowerSprite, ScrollJoiner, ScrollerCanvas, Spreadsheet, TaskQueue, TimelineGrid, TimelineView, UPGRADE_WINDOW, UnzonedRange, VRowGroup, VertResourceViewMixin, View, _filterResourcesWithEvents, applyAll, capitaliseFirstLetter, compareByFieldSpecs, computeGreatestUnit, computeOffsetForSeg, computeOffsetForSegs, copyRect, cssToStr, debounce, detectWarningInContainer, divideDurationByDuration, divideRangeByDuration, durationHasTime, flexibleCompare, getContentRect, getOuterRect, getOwnCells, getRectHeight, getRectWidth, getScrollbarWidths, hContainRect, htmlEscape, intersectRects, isImmuneUrl, isInt, isValidKey, joinRects, multiplyDuration, oldMutateSingle, origApplyOtherRawProps, origEventDefClone, origEventDefToLegacy, origGetSegCustomClasses, origGetSegDefaultBackgroundColor, origGetSegDefaultBorderColor, origGetSegDefaultTextColor, origOnBaseRender, origRemoveElement, origSetElement, parseFieldSpecs, processLicenseKey, proxy, removeExact, renderingWarningInContainer, testRectContains, testRectHContains, testRectVContains, timeRowSegsCollide, vContainRect,
...
FC.schedulerVersion = "1.7.1";

if (FC.internalApiVersion !== 10) {
...
CalendarExtension = (function(superClass) {
  ...
  CalendarExtension.prototype.isFootprintAllowed = function(footprint, peerEventFootprints, constraintVal, overlapVal, subjectEventInstance) {
    ...
    return CalendarExtension.__super__.isFootprintAllowed.apply(this, arguments);
  };
  ...
  CalendarExtension.prototype.eventRangeToEventFootprints = function(eventRange) {
    ...
    if (resourceIds.length) {
      ...
      return CalendarExtension.__super__.eventRangeToEventFootprints.apply(this, arguments);
    }
  };

  CalendarExtension.prototype.getPeerEventInstances = function(subjectEventDef) {
    var peerInstances, subjectResourceIds;
    subjectResourceIds = subjectEventDef.getResourceIds();
    peerInstances = CalendarExtension.__super__.getPeerEventInstances.apply(this, arguments);
  ...
  CalendarExtension.prototype.parseFootprints = function(input) {
    var footprints, j, k, len, len1, plainFootprint, plainFootprints, resourceId, resourceIds;
    plainFootprints = CalendarExtension.__super__.parseFootprints.apply(this, arguments);
  ...
  CalendarExtension.prototype.footprintContainsFootprint = function(outerFootprint, innerFootprint) {
    ...
    return CalendarExtension.__super__.footprintContainsFootprint.apply(this, arguments);
  };

  CalendarExtension.prototype.footprintsIntersect = function(footprint0, footprint1) {
    ...
    return CalendarExtension.__super__.footprintsIntersect.apply(this, arguments);
  };

  CalendarExtension.prototype.buildCurrentBusinessFootprints = function(wholeDay) {
    var anyCustomBusinessHours, flatResources, footprints, j, k, l, len, len1, len2, plainFootprint, plainFootprints, resource;
    ...
    if (anyCustomBusinessHours) {
      ...
      for (k = 0, len1 = flatResources.length; k < len1; k++) {
        ...
        plainFootprints = this._buildCurrentBusinessFootprints(wholeDay, resource.businessHours || this.opt('businessHours'));
        for (l = 0, len2 = plainFootprints.length; l < len2; l++) {
          plainFootprint = plainFootprints[l];
          footprints.push(new ResourceComponentFootprint(plainFootprint.unzonedRange, plainFootprint.isAllDay, resource.id));
        }
      }
      return footprints;
    } else {
      return CalendarExtension.__super__.buildCurrentBusinessFootprints.call(this, wholeDay);
    }
  };

  CalendarExtension.prototype.buildSelectFootprint = function(zonedStartInput, zonedEndInput, resourceId) {
    var plainFootprint;
    plainFootprint = CalendarExtension.__super__.buildSelectFootprint.apply(this, arguments);
    if (resourceId) {
...
View.prototype.filterResourcesWithEvents = function(resources, eventsPayload) {
  var eventInstance, eventInstanceGroup, id, j, k, len, len1, ref, ref1, resourceId, resourceIdHits;
  resourceIdHits = {};
  for (id in eventsPayload) {
    eventInstanceGroup = eventsPayload[id];
    ref = eventInstanceGroup.eventInstances;
    for (j = 0, len = ref.length; j < len; j++) {
      eventInstance = ref[j];
      ref1 = eventInstance.def.getResourceIds();
...
Grid.prototype.getSegResources = function(seg) {
  if (seg.resource) {
    ...
    return this.view.calendar.getEventResources(seg.footprint.getEventLegacy());
  }
};

origEventDefClone = EventDef.prototype.clone;

origEventDefToLegacy = EventDef.prototype.toLegacy;

origApplyOtherRawProps = EventDef.prototype.applyOtherRawProps;

EventDef.prototype.standardPropMap.resourceEditable = true;
...
EventDef.prototype.applyOtherRawProps = function(rawProps) {
  ...
  return origApplyOtherRawProps.apply(this, arguments);
};
...
ResourceViewMixin = {
  ...
  isEventDefDraggable: function(eventDef) {
    return this.isEventDefResourceEditable(eventDef) || View.prototype.isEventDefDraggable.call(this, eventDef);
  },
  isEventDefResourceEditable: function(eventDef) {
    var ref, ref1, ref2;
  ...
  triggerDayClick: function(footprint, dayEl, ev) {
    ...
    return this.publiclyTrigger('dayClick', {
      context: dayEl,
      args: [dateProfile.start, ev, this, this.calendar.resourceManager.getResourceById(footprint.resourceId)]
    });
  },
  ...
  triggerSelect: function(footprint, ev) {
    ...
    return this.publiclyTrigger('select', {
      context: this,
      args: [dateProfile.start, dateProfile.end, ev, this, this.calendar.resourceManager.getResourceById(footprint.resourceId)]
    });
  },
...
ResourceGridMixin = {
  ...
  eventRangeToEventFootprints: function(eventRange) {
    ...
    if (resourceIds.length) {
      ...
      return Grid.prototype.eventRangeToEventFootprints.apply(this, arguments);
    } else {
      return [];
    }
  },
  computeEventDropMutation: function(startFootprint, endFootprint, eventDef) {
    var mutation;
    if (this.view.isEventDefStartEditable(eventDef)) {
      mutation = Grid.prototype.computeEventDropMutation.apply(this, arguments);
    } else {
      mutation = new EventDefMutation();
    }
    if (this.view.isEventDefResourceEditable(eventDef) && startFootprint.resourceId !== endFootprint.resourceId) {
      mutation.oldResourceId = startFootprint.resourceId;
      mutation.newResourceId = endFootprint.resourceId;
    }
    return mutation;
  },
  computeExternalDrop: function(resourceComponentFootprint, meta) {
    var eventDef;
    eventDef = Grid.prototype.computeExternalDrop.apply(this, arguments);
    eventDef.addResourceId(resourceComponentFootprint.resourceId);
    return eventDef;
  },
  computeEventStartResizeMutation: function(startFootprint, endFootprint, eventDef) {
    if (!this.allowCrossResource && startFootprint.resourceId !== endFootprint.resourceId) {
      return;
    }
    return Grid.prototype.computeEventStartResizeMutation.apply(this, arguments);
  },
  computeEventEndResizeMutation: function(startFootprint, endFootprint, eventDef) {
    if (!this.allowCrossResource && startFootprint.resourceId !== endFootprint.resourceId) {
      return;
    }
    return Grid.prototype.computeEventEndResizeMutation.apply(this, arguments);
  },
  computeSelectionFootprint: function(startFootprint, endFootprint) {
    var plainFootprint;
    if (!this.allowCrossResource && startFootprint.resourceId !== endFootprint.resourceId) {
      return;
    }
    plainFootprint = Grid.prototype.computeSelectionFootprint.apply(this, arguments);
    return new ResourceComponentFootprint(plainFootprint.unzonedRange, plainFootprint.isAllDay, startFootprint.resourceId);
  }
};
...
ResourceDayTableMixin = {
  ...
  buildBusinessHourEventFootprints: function(wholeDay) {
    var eventFootprints, j, k, len, len1, plainEventFootprint, plainEventFootprints, ref, resource;
    if (this.flattenedResources) {
      eventFootprints = [];
      ref = this.flattenedResources;
      for (j = 0, len = ref.length; j < len; j++) {
        resource = ref[j];
        plainEventFootprints = this._buildBusinessHourEventFootprints(wholeDay, resource.businessHours || this.view.calendar.opt('businessHours'));
        for (k = 0, len1 = plainEventFootprints.length; k < len1; k++) {
          plainEventFootprint = plainEventFootprints[k];
          eventFootprints.push(new EventFootprint(new ResourceComponentFootprint(plainEventFootprint.componentFootprint.unzonedRange, plainEventFootprint.componentFootprint.isAllDay, resource.id), plainEventFootprint.eventDef, plainEventFootprint.eventInstance));
        }
      }
      return eventFootprints;
    } else {
      return Grid.prototype.buildBusinessHourEventFootprints.apply(this, arguments);
    }
  }
};
...
TimelineView = (function(superClass) {
  ...
  TimelineView.prototype.initialize = function() {
    ...
    return this.addChild(this.timeGrid);
  };
  ...
  TimelineView.prototype.setDateProfileForRendering = function(dateProfile) {
    ...
    return this.timeGrid.setRange(this.renderUnzonedRange);
  };
  ...
  TimelineView.prototype.computeInitialDateScroll = function() {
    ...
    if (this.timeGrid.isTimeScale) {
      ...
      if (scrollTime) {
        ...
        left = this.timeGrid.dateToCoord(this.activeUnzonedRange.getStart().time(scrollTime));
      }
    }
    return {
      left: left
  ...
  TimelineView.prototype.renderEventsPayload = function(eventsPayload) {
    this.timeGrid.renderEventsPayload(eventsPayload);
...
TimelineGrid = (function(superClass) {
  ...
  TimelineGrid.prototype.unzonedStart = null;

  TimelineGrid.prototype.unzonedEnd = null;
  ...
  TimelineGrid.prototype.normalizeComponentFootprint = function(componentFootprint) {
    ...
    if (this.isTimeScale) {
      ...
      dayRange = this.view.computeDayRange(unzonedRange);
      if (this.largeUnit) {
  ...
  TimelineGrid.prototype.rangeUpdated = function() {
    ...
    this.unzonedStart = this.normalizeGridDate(this.unzonedRange.getStart());
    this.unzonedEnd = this.normalizeGridDate(this.unzonedRange.getEnd());
    if (this.isTimeScale) {
      this.unzonedStart.add(this.view.minTime);
      this.unzonedEnd.subtract(1, 'day').add(this.view.maxTime);
    }
    this.unzonedRange = new UnzonedRange(this.unzonedStart, this.unzonedEnd);
    slotDates = [];
    date = this.unzonedStart.clone();
    while (date < this.unzonedEnd) {
  ...
  TimelineGrid.prototype.componentFootprintToSegs = function(footprint) {
    ...
    if (this.computeDateSnapCoverage(footprintStart) < this.computeDateSnapCoverage(footprintEnd)) {
      segRange = normalFootprint.unzonedRange.intersect(this.unzonedRange);
      if (segRange) {
  ...
  TimelineGrid.prototype.getHitFootprint = function(hit) {
  ...
  TimelineGrid.prototype.getSnapUnzonedRange = function(snapIndex) {
    var end, start;
    start = this.unzonedStart.clone();
  ...
  TimelineGrid.prototype.renderDates = function() {
    ...
    for (i = j = 0, len = ref.length; j < len; i = ++j) {
      ...
      this.publiclyTrigger('dayRender', {
        context: this.view,
        args: [date, this.slatEls.eq(i), this.view]
      });
    }
    if (this.follower) {
    ...
    for (j = 0, len = slotDates.length; j < len; j++) {
      ...
      for (row = k = 0, len1 = formats.length; k < len1; row = ++k) {
        ...
        if (isSuperRow) {
          ...
          if (!leadingCell || isInt(divideRangeByDuration(this.unzonedStart, date, labelInterval))) {
            text = date.format(format);
            newCell = this.buildCellObject(date, text, rowUnits[row]);
          } else {
            leadingCell.colspan += 1;
  ...
  TimelineGrid.prototype.slatCellHtml = function(date, isEm) {
    ...
    if (this.isTimeScale) {
      classes = [];
      classes.push(isInt(divideRangeByDuration(this.unzonedStart, date, this.labelInterval)) ? 'fc-major' : 'fc-minor');
    } else {
  ...
  TimelineGrid.prototype.renderNowIndicator = function(date) {
    ...
    if (this.unzonedRange.containsDate(date)) {
  ...
  TimelineGrid.prototype.computeDateSnapCoverage = function(date) {
    ...
    snapDiff = divideRangeByDuration(this.unzonedStart, date, this.snapDuration);
    if (snapDiff < 0) {
      return 0;
  ...
  TimelineGrid.prototype.fgSegHtml = function(seg, disableResizing) {
    ...
    return '<a class="' + classes.join(' ') + '" style="' + cssToStr(this.getSegSkinCss(seg)) + '"' + (eventDef.url ? ' href="' + htmlEscape(eventDef.url) + '"' : '') + '>' + '<div class="fc-content">' + (timeText ? '<span class="fc-time">' + htmlEscape(timeText) + '</span>' : '') + '<span class="fc-title">' + (eventDef.title ? htmlEscape(eventDef.title) : '&nbsp;') + '</span>' + '</div>' + '<div class="fc-bg" />' + (isResizableFromStart ? '<div class="fc-resizer fc-start-resizer"></div>' : '') + (isResizableFromEnd ? '<div class="fc-resizer fc-end-resizer"></div>' : '') + '</a>';
  };
  ...
  TimelineGrid.prototype.renderHelperEventFootprintEls = function(eventFootprints, sourceSeg) {
    var segs;
    segs = this.eventFootprintsToSegs(eventFootprints);
    segs = this.renderFgSegEls(segs);
  ...
  TimelineGrid.prototype.renderEventResize = function(eventFootprints, seg) {
  ...
  TimelineGrid.prototype.renderDrag = function(eventFootprints, seg) {
    var eventFootprint, j, len;
    if (seg) {
...
TimelineGrid.prototype.validateLabelAndSlot = function() {
  ...
  currentUnzonedRange = this.view.currentUnzonedRange;
...
ResourceTimelineView = (function(superClass) {
  ...
  ResourceTimelineView.prototype.renderSkeletonHtml = function() {
    var theme;
    theme = this.calendar.theme;
    return '<table class="' + theme.getClass('tableGrid') + '"> <thead class="fc-head"> <tr> <td class="fc-resource-area ' + theme.getClass('widgetHeader') + '"></td> <td class="fc-time-area ' + theme.getClass('widgetHeader') + '"></td> </tr> </thead> <tbody class="fc-body"> <tr> <td class="fc-resource-area ' + theme.getClass('widgetContent') + '"></td> <td class="fc-time-area ' + theme.getClass('widgetContent') + '"></td> </tr> </tbody> </table>';
  };
  ...
  ResourceTimelineView.prototype.renderEventsPayload = function(eventsPayload) {
    this.timeGrid.renderEventsPayload(eventsPayload);
    this.syncRowHeights();
    return this.updateWidth();
  };
...
ResourceTimelineGrid = (function(superClass) {
  ...
  ResourceTimelineGrid.prototype.componentFootprintToSegs = function(footprint) {
    var calendar, j, len, resourceId, seg, segs;
    segs = ResourceTimelineGrid.__super__.componentFootprintToSegs.apply(this, arguments);
  ...
  ResourceTimelineGrid.prototype.getHitFootprint = function(hit) {
    var componentFootprint;
    componentFootprint = ResourceTimelineGrid.__super__.getHitFootprint.apply(this, arguments);
  ...
  ResourceTimelineGrid.prototype.renderSkeleton = function() {
    var rowContainerEl, theme;
    ResourceTimelineGrid.__super__.renderSkeleton.apply(this, arguments);
    theme = this.view.calendar.theme;
    this.segContainerEl.remove();
    this.segContainerEl = null;
    rowContainerEl = $('<div class="fc-rows"> <table class="' + theme.getClass('tableGrid') + '"> <tbody/> </table> </div>').appendTo(this.bodyScroller.canvas.contentEl);
  ...
  ResourceTimelineGrid.prototype.populateRowBusinessHoursSegs = function(row) {
    var businessHourEventFootprints, businessHourSegs;
    businessHourEventFootprints = this._buildBusinessHourEventFootprints(!this.isTimeScale, row.resource.businessHours || this.opt('businessHours'));
    businessHourSegs = this.eventFootprintsToSegs(businessHourEventFootprints);
  ...
  ResourceTimelineGrid.prototype.renderHelperEventFootprintEls = function(eventFootprints, sourceSeg) {
    var pairs, segs;
    segs = this.eventFootprintsToSegs(eventFootprints);
    segs = this.renderFgSegEls(segs);
    pairs = this.view.pairSegsWithRows(segs);
